...
# Dark VPN / V2Dark (FlyD) — live panel API (+ CloudFront mirror when gem-panel blocks GHA).
DV_API_URL = "https://api.gem-panel.com/api/v1/apps/app-data"
DV_CF_API_URL = "https://d3pwd137ql21v4.cloudfront.net/api/v1/apps/app-data"
...
def http_get_json_exact(url: str, headers: dict[str, str], timeout: int = 45) -> dict:
    """GET JSON using only the given headers (avoids Begzar default header bleed)."""
    ctx = ssl.create_default_context()
    req = urllib.request.Request(url, headers=headers)
    with urllib.request.urlopen(req, timeout=timeout, context=ctx) as resp:
        return json.loads(resp.read().decode("utf-8"))
...
def fetch_dv_wrapper() -> dict:
    """Fetch DarkVPN app-data from live gem-panel, then same API on CloudFront."""
    api_headers = {
        "User-Agent": "okhttp/4.12.0",
    ...
    sources = [
        ("gem-panel", DV_API_URL, api_headers),
        ("cloudfront", DV_CF_API_URL, api_headers),
    ]
    errors: list[str] = []
